...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **FR-1** | Register Account (Name, Email, Password, Status, Field of Study) | Authentication | `register.html`, `auth.js` | `auth.users`, `public.users`, `public.profiles` | TC-1.1 to TC-1.8 | Form input validation, trigger auto-creation in `public.users` and `public.profiles`, duplicate email rejection. | **Implemented, Not Yet Tested** |
| **FR-2** | Login (Role-based redirect, generic anti-enumeration error) | Authentication | `login.html`, `auth.js` | `auth.users`, `public.users` | TC-2.1 to TC-2.8 | Email/password sign-in, role checking (`Student` $\to$ `dashboard.html`, `Admin` $\to$ `admin-jobs.html`), deactivated account block. | **Implemented, Not Yet Tested** |
| **FR-3** | Manage Profile & Dynamic Profile Strength | Profile Management | `profile.html`, `profile.js` | `public.profiles`, `public.profile_skills`, `public.skills` | TC-3.1 to TC-3.6 | Academic background CRUD, multi-tag tech/soft skill chips with delete/add, live Profile Strength meter ($0-100\%$). | **Implemented, Not Yet Tested** |
...
| **FR-9** | Log Out | Authentication | All views, `auth.js` | `auth.users` (Session termination) | TC-9.1 to TC-9.3 | Global sign out button, session invalidation, route protection preventing post-logout back-navigation. | **Implemented, Not Yet Tested** |
...
| **SE-1 / SE-2** | Cross-User Data Isolation | PostgreSQL Row Level Security (RLS) on all 8 tables. | Users cannot read/update foreign profiles, recommendations, or applications. | Foreign user ID query injection attempt (TC-2.8) | **Enforced by RLS (Awaiting TC-2.8 Proof)** |
